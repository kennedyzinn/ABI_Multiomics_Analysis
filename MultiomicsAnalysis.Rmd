...
library(dplyr)
npc_combined <- subset(combined, subset = cell_type == "NPCs")
...
plot_gene_by_time <- function(seurat_obj, gene, time_col = "orig.ident") {
  ...
  # Fetch raw data for boxplots
  df_raw <- FetchData(seurat_obj, vars = c(gene, time_col))
  ...
  # Create plot with boxplot
  ggplot(df_raw, aes(x = .data[[time_col]], y = .data[[gene]])) +
    geom_boxplot(fill = "lightblue", alpha = 0.7) +
    geom_jitter(width = 0.2, alpha = 0.3, size = 0.5, color = "gray40") +  # Optional: show individual points
    geom_line(data = df_summary, aes_string(x = time_col, y = "avg_expr", group = 1),
              size = 1, color = "steelblue") +
    geom_point(data = df_summary, aes_string(x = time_col, y = "avg_expr"),
               size = 3, color = "darkblue") +
    labs(
      title = paste("Expression of", gene, "across time points"),
      x = "Time point",
      y = "Expression"
    ) +
    theme_bw()
...
The plots of each gene's profile across development uses normalized expression data.
...
plot_gene_by_time(npc_combined, "Xist")
```

The UMAP below represents Xist expression in each NPC. Overall there appears to be consistent Xist expression here. Fewer cells appear expressed on the left hand side of the UMAP.

```{r}
FeaturePlot(npc_combined, features = "Xist") +
  ggtitle("Xist Expression in NPCs from E11.5 to E16.5")
```


Kcnn2 enables small conductance calcium-activate potassium channel activity. It is used for modulating chemical synaptic transmission and is active in glutamatergic synapses and postsynaptic membrane (NIH). This gene was a unique top DEG in early neurogensis. However, but unsurprisingly, the plot shows that expression increases throughout neurogenesis, peaking at E14.5 and leveling out after; the most significant increase in expression was detected in the early phases, from E11.5 to E12.5. Most NPCs did not express Kcnn2; hence the absence of boxplots here.

```{r}
plot_gene_by_time(npc_combined, "Kcnn2")
```
The UMAP below represents the expression levels of Kcnn2 for each NPC across all time points. As suspected by the lack of boxplots, most NPCs do not express Kcnn2. Expression seems particularly sparse on the left hand side of the UMAP. Later on in the report, we can observe the different classes of NPCs present here. There appears to be a higher concentration of Kcnn2 expression where GABA-ergic NPCs and glutamatergic NPCs overlap.

```{r}
FeaturePlot(npc_combined, "Kcnn2") +
  ggtitle("Kcnn2 Expression in NPCs from E11.5 to E16.5")
```

Hbb-bs is a top unique DEG in the later phases of neurogenesis. Hbb-bs code for a polyeptide chain found in hemoglobin and is involved in transporting oxygen to peripheral tissues. Peaking in expression at E12.5 and dropping by E13.5, Hbb-bs expression gradually increases from E13.5 to E16.5. Literature suggests that neuronal hemoglobin expression in mature neurons may play a role in neuronal epigenetics, metabolism and function (Brown et al., 2016, Richter et al., 2009). Similar to Kcnn2, many NPCs did not express Hbb-bs.


```{r}
plot_gene_by_time(npc_combined, "Hbb-bs")
```

Hbb-bs expression seems mostly evenly distributed across the UMAP, aside from fewer expressed cells on the left-hand side. The small cluster in the center demonstrates substaintially greater levels of Hbb-bs expression than in other clusters, which may suggest why this cluster is distinct from the rest of the samples.

```{r}
FeaturePlot(npc_combined, "Hbb-bs") +
  ggtitle("Hbb-bs Expression in NPCs from E11.5 to E16.5")
```
...
The raw data of the ATAC-seq data consists of two files; the peaks file and the fragments file. The fragment file is a matrix of five columns; the first three columns are the genomic locations (chromosome, start position and end position), the fourth column is the cell ID and the fifth column is the total number of reads from that cell that are associated with that fragment. The peaks file is a sparse feature by cell matrix where the rows are peaks and the columns are the cells. Each element within this sparse matrix is a  that cell within that locus.
...
Gene activity scores are calculated using the GeneActivity() function from the Signac package. This score is a result of computing the counts per cell in the gene body and promoter regions. Assuming that open chromatin implies transcription, this score imputes gene expression from the accessibility of the gene's regulatory regions. We can compare this metric to the actual gene expression using the RNA data sampled at E16.5.
...
genes_to_test <- c('Pax6', 'Sox2', 'Nes', 'Dcx')
...
for (i in 1:min(5, length(top_5_genes))) {
  ...
  p <- ggplot(plot_data, aes(x = expression, y = accessibility, color = cell_type)) +
    ...
    labs(title = paste(gene, "→ Pax6 Accessibility"),
